    ...
    async def slut(self, ctx):
        if str(ctx.channel) in ecogame_channels:
            ...
            if failrate == 3:
                ...
                risico = eco_data[0] + risico_new

                risico_waarde = randint(1, 100)
                if risico_waarde <= risico or risico >= 91:
                    ...
                    em = discord.Embed(
                        description=f"Je nam een risico om dit te doen met je job. Je bent Ontslagen!\nJe hebt een cooldown van `7 dagen` om terug te solliciteren.",
                        color=embedcolor,
                    ...
                    maxergdb_cursor.execute(f"UPDATE maxerg_economie SET risico = {risico} WHERE user_id = {ctx.author.id}")
                    db_maxerg.commit()
